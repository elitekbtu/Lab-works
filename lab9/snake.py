...
foods = [{"type": "red", "weight": 4}, {"type": "blue", "weight": 2}, {"type": "green", "weight": 1}]
...
def game_loop():
    ...
    while not gamex:
        ...
        pygame.draw.rect(game_display, food["type"], [food_x, food_y, apple_size, apple_size])
        ...
        if current_time - food_last_update >= food_timer:
            food = random.choices(foods, weights=[food["weight"] for food in foods])[0]
            food_x = random.randrange(0, width - apple_size, apple_size)
            food_y = random.randrange(0, height - apple_size, apple_size)
            food_last_update = current_time
